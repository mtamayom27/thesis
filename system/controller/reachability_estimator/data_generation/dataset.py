...
class ReachabilityDataset(data.Dataset):
    ...
    def reachable(self, env_model, src_sample, dst_sample, same_traj, path_l):
        ...
        if over == 1:
            ...
            if overlap_ratios[0] < 0.1 and overlap_ratios[1] < 0.1:
                ...
                reached = 0.0
                title = "Agent more than 0.7 from actual goal. Distance: " + str(np.round(np.linalg.norm(dst_sample[0] - env.xy_coordinates[-1]),1))
            else:
                #Agent did actually reach the goal
                reached = 1.0
                title = "Agent reached the goal. Distance: " + str(np.round(np.linalg.norm(dst_sample[0] - env.xy_coordinates[-1]),1))
        if over == 0:
    ...
    def __getitem__(self,idx):
        ...
        while True:
            ...
            r, dist,coo,ori,first_gv,sample_after_turn = self.reachable(map_name, src_sample, dst_sample, same, path_l)
            if r is None:
...
def save_reachability_samples(filename, nr_samples,traj_file):
    ...
    for i in range(nr_samples):
        ...
        only_dst_imgs = []
        for i in dst_imgs:
            only_dst_imgs.append(i.flatten())
        only_dst_imgs = np.array(only_dst_imgs)
        
        sample = (src_img[2].flatten(),only_dst_imgs.flatten(),r,s[0],s[1],first_gv,img_after_turn[2].flatten(),dist)
...
if __name__ == "__main__":
    ...
    test = True
    if test:
        save_reachability_samples("test1", 1,"test_1.hd5")
        display_samples("test1.hd5")
        save_reachability_samples("test2", 1,"test_2.hd5")
        display_samples("test2.hd5")
        save_reachability_samples("test3", 1,"test_3.hd5")
        display_samples("test3.hd5")
    elif len(sys.argv) == 4:
        _, filename, num_samples, traj_file = sys.argv
        print_debug(sys.argv)
        save_reachability_samples(filename,int(num_samples), traj_file)
    else:
        import time
        start = time.time()
        save_reachability_samples("reachability_fifty_sampels",50,"trajectories_Savinov3.hd5")
        end = time.time()
        print(end - start)