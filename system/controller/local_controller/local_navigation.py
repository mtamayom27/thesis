...
def compute_navigation_goal_vector(gc_network, nr_steps, env, model = "pod", pod=None):
    ...
    if (distance_to_goal_original > 0.3 and distance_to_goal/distance_to_goal_original < update_fraction) or nr_steps == 0:
...
def vector_navigation(env, goal, gc_network, gc_spiking = None, model = "combo",
    ...
    # env.nr_ofsteps=0 #TODO:For Future Work: trigger instant recalculation of the goal vector
    ...
    while n < step_limit and not stop:
        ...
        if exploration_phase:
            ...
            #TODO: parameterize context length and delta T
            # observations with context length k=10 and delta T = 3 
            obs = env.images[::3][-10:]
            [firing_values, created_new_pc] = pc_network.track_movement(gc_network.gc_modules, False,obs, env.xy_coordinates[-1],maze = env.env_model)
            if created_new_pc:
        ...
        if over == -1:
            ...
            if model == "combo" and env.mode == "pod":
                ...
                end_state = "Agent reached the goal. Actual distance: " + str(np.linalg.norm(env.goal_pos - env.xy_coordinates[-1])) + "."
                stop = True
    ...
    if plot_it:
        plot.plotTrajectoryInEnvironment(env, string = end_state)
...
if __name__ == "__main__":
    ...
    if not experiment:
        ...
        vector_navigation(env, goal, gc_network, gc_spiking = target_spiking, model = model, step_limit = float('inf'), plot_it = True)
        ...
        for i in range(0, nr_trials):
            ...
            if simple:
                ...
                vector_navigation(env, list(start), gc_network, gc_spiking = target_spiking, model = model, step_limit = 8000, plot_it = False)
            ...
            # 3B) GENERATING THE GOAL SPIKINGS, NAVIGATING TO THE GOAL, THEN RETURN TO START
                ...
                start_time = time.time()
                vector_navigation(env, list(goal), gc_network, gc_spiking = target_spiking, model = model, step_limit = 8000, plot_it = False)
                actual_error_goal = np.linalg.norm(env.xy_coordinates[-1] - env.goal_pos)
                actual_error_goal_array.append(actual_error_goal)
                env.nr_ofsteps = 0
                vector_navigation(env, list(start), gc_network, gc_spiking = start_spiking, model = model, step_limit = 8000, plot_it = False)
        ...
        def three_trials(model, working_combinations,num_ray_dir, cone, mapping, combine):
            ...
            over,_ = vector_navigation(env, goal, gc_network = gc_network,gc_spiking= target_spiking,model = model,plot_it=False,step_limit=10000)
            if over != 1: return
