...
class CognitiveMap:
    def __init__(self, from_data=False, re_type = "distance", mode = "exploration", connection = ("all","delayed"),env_model = None):
        ...
        # for simulation 
        self.reach_estimator.env_model = env_model
        
        # thresholds for different RE types
        if self.reach_estimator.type == "distance":
            self.connection_threshold = 0.75
        elif self.reach_estimator.type == "neural_network":
            self.connection_threshold = 0.5
        elif self.reach_estimator.type == "simulation":
            self.connection_threshold = 1.0
        if self.reach_estimator.type == "view_overlap":
    ...
    def reachable(self,reach):
        """ Determine whether reachability value meets the threshold """
        if self.reach_estimator.type == "distance":
            return reach < self.connection_threshold
        elif self.reach_estimator.type == "neural_network":
            return reach > self.connection_threshold
        elif self.reach_estimator.type == "simulation":
            return reach >= self.connection_threshold
        elif self.reach_estimator.type == "view_overlap":
            return reach > self.connection_threshold
    ...
    def add_node_to_map(self, p):
        ...
        if self.connection[1] == "instant":
            ...
            self._connect_single_node(p,self.connection[0])
    ...
    def test_place_cell_network(self,env,gc_network,from_data=False):
        ...
        for i,gv in enumerate(pred_gvs):
            ...
            if i%2== 0 or i %3== 0 or i%5 == 0:
                continue
            plt.quiver(env.xy_coordinates[-1][0], env.xy_coordinates[-1][1], gv[0], gv[1], color='grey', angles='xy', scale_units='xy', scale=1,width=0.005)
            plt.quiver(env.xy_coordinates[-1][0]+gv[0], env.xy_coordinates[-1][1]+gv[1],error[i][0],error[i][1],color='red', angles='xy', scale_units='xy', scale=1,width=0.005)
...
if __name__ == "__main__":
    ...
    if testing:
        ...
        # environment setup
        dt = 1e-2
        env = PybulletEnvironment(False,dt,"Savinov_val3", "analytical",buildDataSet=True ,start = list(list(cm.node_network)[1].env_coordinates))
